...
public class DataUtil {
	...
	public static Object[][] getData(String testCase, ExcelReader excel) {
		...
		for (testCaseRowNum = 1; testCaseRowNum <= rows; testCaseRowNum++) {
			System.out.println("test case row num is: " + testCaseRowNum);
			String testCaseName = excel.getCellData(Constants.DATA_SHEET, 1, testCaseRowNum);
		...
		int testRows = 0;
		while (!excel.getCellData(Constants.DATA_SHEET, 1, dataStartRowNum + testRows).equals("")) {
			testRows++;
		}
		...
		int testCols = 0;
		while (!excel.getCellData(Constants.DATA_SHEET, testCols+1, colStartColNum).equals("")) {
			testCols++;
		}
		...
		for (int rNum = dataStartRowNum; rNum < (dataStartRowNum + testRows); rNum++) {
			...
			for (int cNum = 1; cNum <= testCols; cNum++) {
				...
				System.out.println("row num: " + rNum + ", col num " + cNum + " is: " + testData + " with col name is: " + colName);
			}
